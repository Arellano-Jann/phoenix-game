defmodule GameappWeb.BrandLive.FormComponent do
  ...
  def update(%{brand: brand} = assigns, socket) do
    changeset = Database.change_brand(brand)
  ...
  def handle_event("validate", %{"brand" => brand_params}, socket) do
    changeset =
      ...
      |> Database.change_brand(brand_params)
  ...
  defp save_brand(socket, :edit, brand_params) do
    case Database.update_brand(socket.assigns.brand, brand_params) do
      {:ok, brand} ->
  ...
  defp save_brand(socket, :new, brand_params) do
    case Database.create_brand(brand_params) do
      {:ok, brand} ->
        notify_parent({:saved, brand})
